...
def load_wrangle_data():
    """retrieves data from api, converts to dataframe
    and saves as a csv. Adding features and rearranging 
    ...
    df_846['date_text'] = df_846['date'].apply(lambda x: x.strftime("%B %m"))
    ...
    #rename columns
    df_846 = df_846.rename(columns={"geocoding":"geolocation", "links":"src", "pub_id":"id",
                       "description":"desc"})
    ...
    #re-order columns
    df_846 = df_846[['src', 'state', 'city', 'desc', 'tags', 'geolocation', 'title', 'date', 'date_text', 'id', 'lat', 'long']]
